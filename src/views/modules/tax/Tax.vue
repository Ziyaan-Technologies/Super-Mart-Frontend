...
const rateRule = (v: any) => (v !== '' && v !== null && Number(v) >= 0 && Number(v) <= 100) || 'Rate must be between 0 and 100';
...
async function save() {
  ...
  saving.value = true;
  try {
    const payload = { ...form.value, name: form.value.name.trim(), rate: Number(form.value.rate) };
    if (editingId.value) {
      await axios.put(`taxes/${editingId.value}`, payload);
      alerts.success('Tax has been updated!');
    } else {
      await axios.post('taxes', payload);
      alerts.success('Tax has been created!');
    }
    drawer.value = false;
    list.refresh();
  } catch (error) {
...
async function remove() {
  try {
    await axios.delete(`taxes/${deleteId.value}`);
    alerts.success('Tax has been deleted!');
    list.refresh();
  } catch (error) {
    alerts.fail(error);
  }
...
<template>
  <v-row>
    <v-col cols="12">
      <UiParentCard title="Taxes" icon="mdi-percent-outline">
        <ListToolbar :total="list.totalItems.value" label="Total Taxes" search-placeholder="Search by name..."
          add-label="Add New Tax" :can-add="can('taxes_create', 'Tax')" exportable :exporting="list.exporting.value"
        ...
        <v-data-table :loading="list.loading.value" :items-per-page="list.itemsPerPage.value" :headers="headers" :items="list.items.value"
          ...
          <template v-slot:item.actions="{ item }">
            <v-btn v-if="$can('taxes_edit', 'Tax')" icon="mdi-pencil-outline" color="#EFF0F1" size="small" class="me-2" @click="openForm(item)"></v-btn>
            <v-btn v-if="$can('taxes_delete', 'Tax')" icon="mdi-delete-outline" color="#FFEFEF" size="small" class="text-error" @click="openDelete(item)"></v-btn>
          </template>
  ...
  <RightDrawer ref="drawerRef" v-model="drawer" :title="editingId ? 'Edit Tax' : 'Add New Tax'" :submit-label="editingId ? 'Update Tax' : 'Create Tax'"
    ...
    <v-row>
      <v-col cols="12" class="pt-4">
        ...
        <v-text-field v-model="form.name" :rules="[(v: string) => !!v?.trim() || 'Name is required']" placeholder="e.g. GST 18%" hide-details="auto" />
      </v-col>
      <v-col cols="12">
        <v-label class="text-subtitle-1 pb-2 text-lightText">Rate (%)</v-label>
        <v-text-field v-model="form.rate" type="number" min="0" max="100" :rules="[rateRule]" hide-details="auto" />
      </v-col>
  ...
  <DeleteDialog v-model="deleteOpen" message="Are you sure you want to delete this tax?" @confirm="remove" />
</template>
